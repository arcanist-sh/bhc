...
impl Compiler {
    ...
    fn core_lower_with_constructors(
        ...
        if self.session.options.opt_level != bhc_session::OptLevel::None {
            ...
            // Numeric list fusion: rewrite `sum`/`product`/`foldl'` over
            // `enumFromTo` (optionally through a manifestly Int->Int `map f`) into
            // a hoisted top-level counting loop, eliminating the intermediate cons
            // list. Runs last so no later simplifier pass inlines the recursive loop.
