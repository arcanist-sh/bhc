...
**`LaTeX.Inline` diagnosed, NOT fixed (2026-07-23, multi-layer — deferred).** `doTerm Translations.Figure` fails: `Translations.Figure` typed as `t->t->t->Block` (generic ctor fallback). Root cause: `module X` re-exports are SKIPPED in export collection (`loader.rs` `collect_exports` had `Export::Module => skip`; also the driver's `build_module_exports_from_hir` reads `hir.exports` which likely drops `module X`). `Text.Pandoc.Translations` re-exports `module Text.Pandoc.Translations.Types` (which defines `data Term`), so its constructors never reach importers. DIRECT `import Text.Pandoc.Translations.Types` works — proof it's purely the re-export gap. Implementing it in `collect_exports` alone did NOT move the score (the full-tree resolver uses the driver/HIR path); a real fix needs `module X` re-exports preserved through HIR → driver export building (multi-layer) for ~1 module. Deferred.
